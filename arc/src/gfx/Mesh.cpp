...
arc::Mesh::Mesh(bool isStatic, int maxVertices, int maxIndices, arc::VertexAttributes& attributes): _attributes(attributes) {
    _attributes = attributes;
    _vertices = new VertexBuffer(isStatic, maxVertices, _attributes);
    _indices = new IndexBuffer(isStatic, maxIndices);
}
arc::Mesh::Mesh(bool isStatic, int maxVertices, int maxIndices, arc::VertexAttribute& attributes...): _attributes(attributes) {
    _attributes = attributes;
    _vertices = new VertexBuffer(isStatic, maxVertices, _attributes);
    _indices = new IndexBuffer(isStatic, maxIndices);
}
...
void arc::Mesh::setVertices(std::vector<float> &vertices) {
...
void arc::Mesh::render(arc::ShaderProgram *shader, int primitiveType) {
    render(shader, primitiveType, 0, _indices->getNumMaxIndices() > 0 ? _indices->getNumIndices() : _vertices->getNumVertices(), autoBind);
}
...
void arc::Mesh::render(arc::ShaderProgram *shader, int primitiveType, int offset, int count, bool autoBind) {
    ...
    {
        auto orr = offset * 2;
        glDrawElements(primitiveType, count, GL_UNSIGNED_SHORT, &orr);
    }
    else
    {
